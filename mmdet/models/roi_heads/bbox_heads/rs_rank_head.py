...
class RSRankHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 loss_rank=dict(type='RankSort'),
                 delta=0.5,
                 conv_cfg=None,
                 norm_cfg=None,
                 init_cfg=None
                 ):
        super(RSRankHead, self).__init__(init_cfg)
    ...
    def loss(self,
        ...
        """
        参考 RankBasedShared2FCBBoxHead 进行修改
        :param rank_score:
        :param labels:
        :param label_weights:
        :return:
        ...
        flat_preds = rank_score.reshape(-1)
        if rank_score is not None:
            if rank_score.numel() > 0:
                IoU_targets = iou_targets[iou_targets > 0]

                if self.loss_rank.__class__.__name__ == "RankSort":
                    ...
                    losses['sorting_loss'] = sorting_loss
                else:
                    losses_ranking = self.loss_rank(flat_preds, flat_labels, IoU_targets)
                    losses['losses_ranking'] = losses_ranking
    ...
    def _get_target_single(self, rank_scores, proposal, gt_bboxes, gt_labels):
        ...
        labels = rank_scores.new_full((num_bboxes,), self.num_classes, dtype=torch.long)
        label_weights = rank_scores.new_full((num_bboxes,), 0, dtype=torch.long)
        iou_targets = rank_scores.new_full((num_bboxes,), 0, dtype=torch.float32)
        if gt_labels.size(0) == 0:
            return labels, label_weights, iou_targets
        ...
        if pos_inds.any():
            labels[pos_inds] = gt_labels[max_gt_inds[pos_inds]]
            # TODO: 为了方便给放到这里
            label_weights[pos_inds] = 1
            iou_targets[pos_inds] = max_iou[pos_inds]
        if neg_inds.any():
            # labels[neg_inds] = 0
    ...
    def get_targets(self,
        ...
        labels_list, label_weights_list, iou_targets_list = multi_apply(
