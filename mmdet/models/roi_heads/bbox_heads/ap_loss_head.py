...
class APLossHead(BaseModule):
    """
    AP-loss <https://github.com/cccorn/AP-loss/blob/master/lib/model/aploss.py>
    ...
    def __init__(self,
                 ...
                 norm_cfg=None,
                 init_cfg=None):
        super(APLossHead, self).__init__(init_cfg)
    ...
    def loss(self,
        ...
        if rank_score is not None:
            if rank_score.numel() > 0:
                ...
                if self.loss_rank.__class__.__name__ == 'AutoGradAPLoss':
                    loss_rank_ = self.loss_rank.apply(rank_score, targets)
                else:
                    loss_rank_ = self.loss_rank(rank_score, targets)
    ...
    def _get_target_single(self, rank_scores, proposal, gt_bboxes, gt_labels):
        """
        reference from <github.com/cccorn/AP-loss/blob/master/lib/model/aploss.py>
        """

        if gt_labels.size(0) == 0:
            return rank_scores.new_zeros(rank_scores.size()), None

        IoU = bbox_overlaps(proposal[:, :4], gt_bboxes)  # num_anchors x num_annotations
        IoU_max, IoU_argmax = torch.max(IoU, dim=1)  # num_anchors x 1

        # compute the loss for classification
        targets = rank_scores.new_ones(rank_scores.size()) * -1

        ######
        gt_IoU_max, gt_IoU_argmax = torch.max(IoU, dim=0)
        # torch.where 只给一个参数用相当于nonzero了
        gt_IoU_argmax = torch.where(IoU == gt_IoU_max)[0]
        # torch.ge：逐元素比较input和other，即是否 input>=other
        positive_indices = torch.ge(rank_scores.new_zeros(IoU_max.size()), 1)  # 初始化和IoU_max大小相同，全为0
        positive_indices[gt_IoU_argmax.long()] = True  # 代表哪些roi匹配到了gt，用原gt_IoU_argmax，不一样吗
        ######

        positive_indices = positive_indices | torch.ge(IoU_max, 0.5)  # pos的iou应大于0.5
        # torch.lt：逐元素比较input和other，即是否 input < other
        negative_indices = torch.lt(IoU_max, 0.4)  # 最大iou小于0.4的做neg
        # assigned_annotations_label = gt_labels[IoU_argmax]    # 这句话似乎有问题
        assigned_annotations_label = gt_labels[IoU_argmax[positive_indices]]

        # targets原本为-1
        targets[negative_indices, :] = 0
        targets[positive_indices, :] = 0
        targets[positive_indices, assigned_annotations_label] = 1  # 正样本对应label位置置为1
